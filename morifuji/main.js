...
function main() {
    ...
    for (var i = 0; i < row; i++) {
        for (var j = 0; j < col; j++) {
            blocks[i * col + j] = new Block({ x: 160 + j * 80, 　y: 40 + i * 40, 　w: 80, 　h: 40 });
        }
    }
    // アニメーション
    setInterval(function() {
        if (game.status != 'play') {
            ...
            if (game.status == 'gameover') {
                ...
                game.ctx.fillText('Score : ' + game.score.point, 160, 400);
            var img = new Image();
            img.src = "clear.png";
            game.ctx.drawImage(img, 0, 100, 640, 200);
            }
            return;
        }
        game.time++; // 時間加算
        ...
        var img = new Image();
        img.src = "shimizu.jpg";
        game.ctx.drawImage(img, 200, 40, 200, 200);
        paddle.draw(); //バドル描画
        ball.draw(); //ボール描画
        // ブロック描画
        for (var i = 0; i < row; i++) {
